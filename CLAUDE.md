...
ARMADERO — a single-page portfolio landing site for a fictional premium custom closet/wardrobe storage-systems brand (Next.js/React/TypeScript/Tailwind, content in Ukrainian). It is one long `page.tsx` composed of section components, not a multi-page app. The brand went through an earlier repositioning (from an original home-cinema-seating concept to the current wardrobe/closet niche) — that pivot is fully carried out in code: brand name is Armadero, copy/photos are wardrobe-specific, and the base palette is a white background (see `globals.css` below). Internal-only identifiers (e.g. `INTRO_SEEN_KEY = "vellaro-intro-seen"` in `src/lib/intro.ts`) still carry the old "vellaro" name — harmless since they're not user-visible, not a sign anything is incomplete.
